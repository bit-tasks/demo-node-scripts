...
const run = async (gitUserName, gitUserEmail, wsdir) => {
  ...
  if (statusOutput) {
    execSync(`git config --global user.name "${gitUserName}"`, { cwd: wsdir, shell: '/bin/bash' });
    execSync(`git config --global user.email "${gitUserEmail}"`, { cwd: wsdir, shell: '/bin/bash' });
    execSync(`git checkout -b ${branchName}`, { cwd: wsdir, shell: '/bin/bash' });
    execSync("git add .", { cwd: wsdir, shell: '/bin/bash' });
    execSync(`git commit -m "Update Bit envs and outdated dependencies"`, { cwd: wsdir, shell: '/bin/bash' });
    execSync(`git push origin ${branchName} --force`, { cwd: wsdir, shell: '/bin/bash' });
